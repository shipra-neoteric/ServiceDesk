...
### Verified against a real MongoDB replica set — two real, silent bugs found and fixed

The Atlas switch above was originally only schema-validated, never run. This pass actually stood up a local MongoDB replica set (`mongodb-memory-server`, not committed — a real Atlas cluster behaves identically for this purpose) and ran the full backend test suite and a live UI session against it. That surfaced two real, previously-invisible bugs — invisible specifically because SQLite/Postgres don't have this behavior, so nothing before this could have caught them without running against actual MongoDB:

1. **`field: null` filters don't match fields that were simply never written on MongoDB** (they only match fields explicitly present with value `null` — a genuine Mongo/Prisma quirk, confirmed by direct experiment against a live instance). This silently broke: the "no open site visit to complete" lookup (site visits could never be completed once this code path was hit for real), the NO_OWNER attention rule (freshly-raised jobs never flagged as unassigned), the HOLD_REVIEW_EXPIRED rule and resume's "close the open hold/pause" lookups (holds and SLA pauses could never be resumed/closed), the global-holiday lookup used by SLA business-hours math, and the SLA-definition bootstrap's dedupe-before-insert (every bootstrap run would have added a duplicate GLOBAL/PRIORITY SLA row instead of replacing the old one). Fixed by writing these fields as explicit `null` at creation time everywhere they're later queried as `null`, documented inline at each site since it's exactly the kind of thing a future cleanup would "simplify" back into the bug.
2. **`createMany({ data: [] })` throws on MongoDB** ("No documents provided to insert_many") instead of being the harmless no-op it is on SQL databases. Found live: editing a user down to zero roles or zero project-access rows in the new Users screen (below) 500'd. Same latent bug existed in the role→permissions editor and the roles/permissions seed script. Fixed by guarding every `createMany` call site with a length check.

All 55 backend tests (54 + a new regression test for the hold/resume bug, `test/holdResume.test.ts`) pass against the real replica set; a full manual pass through the new Users UI (create → edit → reset password → delete blocked-by-history → deactivate-instead → reactivate) was also verified live, including a Playwright-driven browser session with screenshots. The full committed Playwright e2e suite was started against this same setup but not confirmed to completion (too slow in this sandbox to finish within a reasonable session) — worth re-running against a real Atlas cluster before go-live, but no further Mongo-specific defect is expected given the backend suite's full coverage of the same code paths.

### User Management, from read-only list to full CRUD

The Users screen (`/users`) was previously a read-only table — the backend already had create/edit/deactivate endpoints, but nothing in the frontend called them. It now supports:

- **Add User**: name, email, phone, employee ID, department, designation, initial password, one or more roles, and per-project access (`FULL`/`READ_ONLY`) — a drawer form (`UserFormDrawer.tsx`) matching the same pattern as the Job Card creation drawer.
- **Edit User**: the same form, prefilled, plus an Active/Inactive toggle (email is intentionally not editable — it's the login identifier).
- **Reset Password** (`ResetPasswordModal.tsx`): a new, separate action and permission (`user.reset_password`) — deliberately not folded into `user.edit`, since setting someone else's credential is more sensitive than editing their name. Includes a "Generate" button for a random password.
- **Delete User** (`DeleteUserModal.tsx`, new `DELETE /users/:id`, permission `user.delete`): a real hard delete, but only when the account has zero activity anywhere in the system (no Job Cards created, comments, attachments, approvals, assignments, site visits, materials, verifications, or reopen events). MongoDB enforces no foreign-key constraints, so this check — not the database — is what stops a delete from silently orphaning another record's author/actor reference. Deleting a user with any history is rejected with a 409 and the UI offers a one-click "Deactivate Instead," which preserves their history and is reversible.
- Stat cards (Total/Active/Inactive) double as quick filters, plus a search box across name/email/role — same `KpiCard`/`Toolbar` components the Dashboard and Job Cards list already use.
- Both new permissions default to Master-Admin-only, matching the rest of `user.*` (see `PERMISSIONS.md`).

Not built as part of this: a per-user granular permission override (permissions are still assigned per-Role, not per-User — editing what a Role itself grants isn't exposed in any UI either, tracked as an existing gap below) and self-service "forgot password" (there's still no email-sending capability in this codebase; password reset is admin-initiated only).
...
- **The full Playwright e2e suite has not been re-confirmed against MongoDB** — the backend test suite and a full manual UI pass have (see "Verified against a real MongoDB replica set" above), but the committed 78-test Playwright suite was started against a live Mongo-backed setup and not confirmed to completion in this session. Re-run it before go-live.
...
- **55 backend tests** passing (Vitest + Supertest, confirmed against a real local MongoDB replica set — not just SQLite) — up from 30 at the start of the previous pass: workflow transitions, business-hours SLA math, project/attachment-scope isolation, evidence-gated completion, the full multi-stage bookkeeping fix, 8 Process Coordinator rules, 7 legacy-import scenarios, and a new hold/resume regression test (see "Verified against a real MongoDB replica set" above).
- **76 Playwright tests passing** (2 skipped when seed data doesn't happen to have an engineer with assignments — a defensive skip, not a failure) across `chromium` (desktop) and a `Pixel 5` mobile profile, including a dedicated axe-core accessibility pass and a dark-mode persistence check. Last confirmed against the pre-Atlas SQLite datasource — not yet re-confirmed to completion against MongoDB (see above).
...
1. Point `DATABASE_URL` at your production MongoDB Atlas cluster, run `npm run prisma:deploy` (`prisma db push`) and `npm run bootstrap` (creates roles/permissions/workflow templates and, if `BOOTSTRAP_ADMIN_EMAIL`/`PASSWORD` are set, the first Master Admin account — see `src/seed/bootstrap.ts`), then re-run the full Playwright suite against it to completion before trusting that pass count again. See `DEPLOYMENT.md`.
