...
4. Frontend `PermissionGate` hides/disables controls for UX only — every gated action is re-checked in (1)-(3) server-side. Tested explicitly (`apps/api/test/projectScope.test.ts`): a user without access to Project B gets `404` on Project B's job even when given a valid, well-formed job ID and a valid session.
